...
export default function ParamSelector({ label, options, selected, onSelect }: ParamSelectorProps) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-[10px] md:text-[9px] uppercase tracking-[0.2em] text-text-muted font-semibold">{label}</p>
        ...
        {options.map((opt) => {
          ...
          return (
            <button
              ...
              className={cn(
                "px-3.5 py-2 md:px-3 md:py-1.5 rounded-md text-[11px] md:text-[10px] font-medium transition-all border",
                isActive
